...
public class U22ListingsStepDefs extends CucumberTestProvider {
  ...
  public void iCreateAnotherListingWithQuantityAndPrice(int quantity, float price)
      throws Exception {
    CreateListingDto listing = new CreateListingDto();
    listing.setInventoryItemId(inventoryItemId);
    listing.setQuantity(quantity);
  ...
  public void iListOfTheseForSale(int quantity) throws Exception {
    ...
    JsonNode response = makePostRequestGetJson("/businesses/" + inventoryItemId + "/listings", listing,
        status().isCreated());
    listingId = Long.parseLong(response.get("listingId").toString());
  }

  @Then("i can see the price is generated by the inventory price of {double} * the listing quantity of {int}")
  public void iCanSeeThePriceIsGeneratedByTheInventoryPriceOfTheListingQuantityOf(
      double pricePerItem,
      int quantity) throws Exception {
    MvcResult response = mockMvc.perform(get("/businesses/" + businessId + "/listings"))
        .andReturn();

    double expectedTotal = pricePerItem * quantity;
    Assertions.assertTrue(
        response.getResponse().getContentAsString().contains("\"price\":" + expectedTotal));
  }
