...
public class CountryDataFetcherService {
  ...
  /**
   * Reads a file containing an API response from resources folder and generates the country hash
   * map
   *
   * @throws IOException
   */
  protected void loadCountryData() throws IOException {
    InputStream stream = this.getClass().getClassLoader().getResource("countryData.json")
        .openStream();
  ...
  /**
   * Gets a hash map with the country name as key and the currency that should be used in that
   * country
   *
   * @return hash map
   */
  ...
  /**
   * Checks if country exists (or if the REST Api returns that country and there is a currency we
   * can use)
   *
