...
public class TransactionController {
   ...
   * @return pair of dates transformed so that:
   *  - Start date set to minimum date if not given, and time to start of day
   *  - End date set to the current date if not given
   *    - End time is transformed to last second
   *  - Times are in UTC time as that is what is used elsewhere in the backend. There is no concept of a
   *    local timezone for businesses or users so it makes the most sense to just not send the time in the frontend
   */
  private Pair<ZonedDateTime, ZonedDateTime> transformDateRange(LocalDate startDate, LocalDate endDate) {
    ...
    ZonedDateTime   transformedEndDate = endDate.plusDays(1).atStartOfDay(ZoneOffset.UTC).minusSeconds(1);
  ...
  public ResponseEntity<Object> viewTransactionHistory(
      ...
      @Parameter(description = "Start date for transaction history. Time set to start of day in the UTC timezone")
          @RequestParam(required = false)
          LocalDate startDate,
      @Parameter(description = "End date for transaction history. Time set to end of day in the UTC timezone")
          @RequestParam(required = false)
          LocalDate endDate)
    ...
    return new ResponseEntity<>(
        transactionService.getTransactionData(id, dateRange.getLeft(), dateRange.getRight(), TransactionGranularity.DAY),
        HttpStatus.OK
    );
  }
}
