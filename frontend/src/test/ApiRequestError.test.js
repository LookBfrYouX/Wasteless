...
describe("createFromMessageMap", () => {
  test("no response default handler", () => {
    expect(ApiRequestError.createFromMessageMap(undefined,
        {}).userFacingErrorMessage)
    .toEqual(
        ApiRequestError.DEFAULT_MESSAGES_MAP[ApiRequestError.NO_RESPONSE_KEY]);
  });
  test("no response given handler", () => {
    expect(ApiRequestError.createFromMessageMap(undefined,
        customMessages).userFacingErrorMessage)
    .toEqual("A");
  });

  test("unknown response default handler", () => {
    expect(ApiRequestError.createFromMessageMap(getError(1000),
        {}).userFacingErrorMessage)
    .toEqual(ApiRequestError.DEFAULT_MESSAGES_MAP[ApiRequestError.DEFAULT_KEY](
        getError(1000)));
  });
  ...
  test("unknown response given handler", () => {
    expect(ApiRequestError.createFromMessageMap(getError(200),
        customMessages).userFacingErrorMessage)
    .toEqual("200");
  });

  test("known response default handler", () => {
    expect(ApiRequestError.createFromMessageMap(getError(401),
        {}).userFacingErrorMessage)
