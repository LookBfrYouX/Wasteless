...
describe("arrow behaviour", () => {
  ...
  const setup = async (suggestions = undefined) => {
    if (suggestions == undefined) suggestions = defaultSuggestions;
  ...
  test("arrow down", async () => {
    ...
    for(const index of [0, 1, 2, 2]) {
      await input.trigger("keydown", { code: "ArrowDown", preventDefault: jest.fn });
  ...
  test("arrow up", async () => {
    ...
    for(const index of [1, 0, -1, -1]) {
      await input.trigger("keydown", { code: "ArrowUp", preventDefault: jest.fn });
      expect(wrapper.vm.index).toBe(index);
    }
  });
  ...
  test("enter", async () => {
    ...
    expect(wrapper.emitted("input")[0][0]).toBe(defaultSuggestions[1].toString());
  ...
  test("blur then focus doesn't causes suggestion list to disappear", async () => {
    const { input } = await setup();
