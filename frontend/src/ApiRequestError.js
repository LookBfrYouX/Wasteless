...
export class ApiRequestError extends Error {
  ...
  static createFromMessageMap(error, errorMessages = undefined) {
    ...
    const callOrGet = (key) => {
      ...
      const messageOrFunction = (errorMessages[key] !== undefined)
          ? errorMessages[key] : this.DEFAULT_MESSAGES_MAP[key];
      // Check if it is a function; if so, call the function with the error message
      return typeof messageOrFunction == "string" ? messageOrFunction
    ...
    if (error === undefined || error.response === undefined) {
      ...
      if (errorMessages[code] !== undefined || this.DEFAULT_MESSAGES_MAP[code]
          !== undefined) {
